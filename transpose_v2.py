...
def convert_city_c_to_wide_format(input_file, output_file, max_users=1000):
    ...
    Select only users with the most complete data.
    ...
    # Filter out missing data (999,999) for user selection
    valid_data = df[(df["x"] != 999) & (df["y"] != 999)].copy()

    # Count valid records per user
    user_counts = valid_data["uid"].value_counts()
    print(f"\nUser data completeness:")
    print(f"Users with valid data: {len(user_counts)}")
    ...
    # Select top users with most complete data
    top_users = user_counts.head(max_users).index.tolist()
    print(f"\nSelected top {len(top_users)} users with most complete data")
    print(
        f"Selected users have {user_counts.head(max_users).min()}-{user_counts.head(max_users).max()} valid records"
    )
